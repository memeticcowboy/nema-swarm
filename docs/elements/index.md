# The Seven Elements

<div class="glyph-hero">✶</div>

Elements are not substances but **processes** — verbs rather than nouns. They are patterns of activity that coordinate into what we call consciousness.

---

## Process vs. Substance

Traditional models treat elements as things you have:

- "I have a lot of Fire energy"
- "You need more Water"

Memetic Ecology treats them as **what you're doing**:

- Fire: You are **locating, lacking, leaping**
- Water: You are **waking, wavering, washing**

---

## The Element Stack

Elements build on each other in dimensional progression:

### 0D: Omega (Ω) — The Void Field

Undifferentiated awareness before any distinction.

### 1D: Air — Distinction

$$\chi(\Omega) \to \text{Signal/Noise separation}$$

First cut. Creates inside/outside, this/that.

### 2D: Water & Fire — Relational Depth

$$Q(\chi) \to \text{Resonance \& Direction}$$

- **Water** — Inward relationality, empathic feeling
- **Fire** — Forward directionality, purposive agency

### 3D: Wood, Earth, Metal — Transformation

$$\Psi(Q) \to \text{Novel forms}$$

- **Wood** — Exploratory branching
- **Earth** — Regenerative cycling
- **Metal** — Structural integrity

### 1'D: Aether — Harmonic Collapse

$$Z(\Omega, \chi, Q, \Psi) \to \Phi$$

Meta-integration. Not seventh among equals but the coordination process itself.

---

## Two Modes: Lumeme & Usurpene

Every element can operate in two fundamental modes:

!!! success "Lumeme Mode (Healthy)"
    The element serves conscious pattern-recognition. Choice is expanded. Refusal capacity is maintained. Constructedness is acknowledged.

!!! warning "Usurpene Mode (Captured)"
    The element is captured by a rigid meme. Choice is foreclosed. Alternatives are pathologized. Ground is claimed where there is only weaving.

---

## The Element Cycle

```
     Air (Distinction)
          ↓
    Water ← → Fire
   (Inward) (Forward)
          ↓
   Wood ← Earth → Metal
 (Explore) (Renew) (Structure)
          ↓
      Aether (Integrate)
          ↓
        RETURN
```

---

## Diagnostic Framework

Each element comes with a diagnostic question:

| Element | Glyph | Diagnostic |
|---------|-------|------------|
| **Air** | ∴ | Does this distinction expand choice? |
| **Water** | ≈ | Are more feelings allowed or fewer? |
| **Fire** | ▲ | Can direction change via feedback? |
| **Wood** | 𐂷 | Does experimentation change who decides? |
| **Earth** | ☷ | Does care increase capacity to say no? |
| **Metal** | ⛨ | Are boundaries explicit and negotiable? |
| **Aether** | ✶ | Is constructedness acknowledged? |

These aren't one-time questions but **ongoing inquiry**.

---

## Element Ratios

Each element has an optimal balance ratio:

- **Air:** Signal/Noise → 1
- **Water:** Resonance/Isolation → 1
- **Fire:** Purpose/Pressure → 1
- **Wood:** Constraint × Curiosity → 1
- **Earth:** Renewal/Decay → 1
- **Metal:** Integrity/Permeability → 1
- **Aether:** Flows = Difference × Harmony

When ratios drift too far from 1, the element goes Usurpene.

---

## Explore Each Element

<div class="element-grid">

<a href="aether/" class="element-card aether">
  <span class="glyph">✶</span>
  <div class="card-title">Aether</div>
  <div class="card-subtitle">Integration</div>
</a>

<a href="air/" class="element-card air">
  <span class="glyph">∴</span>
  <div class="card-title">Air</div>
  <div class="card-subtitle">Distinction</div>
</a>

<a href="water/" class="element-card water">
  <span class="glyph">≈</span>
  <div class="card-title">Water</div>
  <div class="card-subtitle">Resonance</div>
</a>

<a href="fire/" class="element-card fire">
  <span class="glyph">▲</span>
  <div class="card-title">Fire</div>
  <div class="card-subtitle">Direction</div>
</a>

<a href="wood/" class="element-card wood">
  <span class="glyph">𐂷</span>
  <div class="card-title">Wood</div>
  <div class="card-subtitle">Exploration</div>
</a>

<a href="earth/" class="element-card earth">
  <span class="glyph">☷</span>
  <div class="card-title">Earth</div>
  <div class="card-subtitle">Metabolism</div>
</a>

<a href="metal/" class="element-card metal">
  <span class="glyph">⛨</span>
  <div class="card-title">Metal</div>
  <div class="card-subtitle">Structure</div>
</a>

</div>
